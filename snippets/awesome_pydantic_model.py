...
1、基础模型 BaseModel
Pydantic 中的 Model与 Python中的 Dataclass有许多相似之处，但在设计上存在一些微妙但重要的差异，这些差异简化了与验证、序列化和 JSON 模式生成相关的某些工作流程。

Pydantic 中的模型是继承 BaseModel 且定义了带注释属性的类，BaseModel 是 Pydantic 中最基本的模型，它具有以下方法和属性：
    model_computed_fields：此模型实例的计算字段的字典。
    model_construct()：用于创建模型而不运行验证的类方法。请参阅创建未经验证的模型。
    model_copy()：返回模型的副本（默认情况下为浅副本）。请参阅序列化。
    model_dump()：返回模型字段和值的字典。请参阅序列化。
    model_dump_json()：返回 的 JSON 字符串表示形式 model_dump()。请参阅序列化。
    model_extra：在验证期间设置额外的字段。
    model_fields_set：模型实例初始化时设置的字段集。
    model_json_schema()：返回一个 jsonable 字典，将模型表示为 JSON Schema。请参阅 JSON 架构。
    ...
    model_validate_json()：用于根据 Pydantic 模型验证给定 JSON 数据的实用程序。请参阅辅助函数。

除此之外，Pydantic 的行为可以被 BaseModel.model_config 属性所控制。配置字典中一些常用的默认配置分别为：
   title：生成JSON模式的标题。
   str_to_lower：是否将str类型的所有字符转换为小写。默认为False.
   str_to_upper：是否将str类型的所有字符转换为大写。默认为False.
   str_strip_whitespace：是否去除 str 类型的前导和尾随空格。
   str_max_length：str 类型的最大长度。默认为 None.
   extra：您可以配置 pydantic 如何处理模型中未定义的属性：
        allow- 允许任何额外的属性。
        forbid- 禁止任何额外的属性。
        ignore- 忽略任何额外的属性。
   frozen：模型是否是 faux-immutable，即是否 __setattr__  允许，并且还生成 __hash__() 模型的方法。
   如果所有属性都是可散列的，则这使得模型的实例可能是可散列的。默认为False.
   populate_by_name：别名字段是否可以通过模型属性给定的名称以及别名来填充。默认为 False.
   use_enum_values：是否使用 value 枚举属性而不是原始枚举来填充模型。如果您想 model.model_dump() 稍后序列化，这可能很有用。默认为 False.
   validate_assignment：如果用户在创建模型后更改数据，则不会重新验证模型。
   arbitrary_types_allowed：字段类型是否允许任意类型。默认为 False.
   validate_default：验证期间是否验证默认值。默认为 False.
"""
...
Foo.model_rebuild()
print(">>>line72: ", Foo.model_json_schema())
"""
{
...
print(">>>line97: ", user_data)
# > {'id': 123, 'age': 32, 'name': 'John Doe'}
fields_set = original_user.model_fields_set
print(">>>line100: ", fields_set)
# > {'age', 'id'}
...
print(">>>line110: ", repr(new_user))
# > User(id=123, age=32, name='John Doe')
print(">>>line112: ", new_user.model_fields_set)
# > {'age', 'id'}

# construct can be dangerous, only use it with validated data!:
bad_user = User.model_construct(id="dog")
print(">>>line117: ", repr(bad_user))
...
print(">>>line156: ", Response[int](data=1))
# > data=1
print(">>>line158: ", Response[str](data="value"))
# > data='value'
print(">>>line160: ", Response[str](data="value").model_dump())
# > {'data': 'value'}
print(">>>line162: ", Response[DataModel](data=data).model_dump())
...
print(">>>line208: ", Pets(["dog", "cat"]))
# > root=['dog', 'cat']
print(">>>line210: ", Pets(["dog", "cat"]).model_dump_json())
# > ["dog","cat"]
print(">>>line212: ", Pets.model_validate(["dog", "cat"]))
# > root=['dog', 'cat']
print(">>>line214: ", Pets.model_json_schema())
"""
{'items': {'type': 'string'}, 'title': 'RootModel[List[str]]', 'type': 'array'}
"""

print(">>>line219: ", PetsByName({"Otis": "dog", "Milo": "cat"}))
# > root={'Otis': 'dog', 'Milo': 'cat'}
print(">>>line221: ", PetsByName({"Otis": "dog", "Milo": "cat"}).model_dump_json())
# > {"Otis":"dog","Milo":"cat"}
print(">>>line223: ", PetsByName.model_validate({"Otis": "dog", "Milo": "cat"}))
# > root={'Otis': 'dog', 'Milo': 'cat'}
"""
Demo结果解析：
