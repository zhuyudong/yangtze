...
每日自动抓取于 V2EX 和电鸭社区的招聘信息，使用 Google AI 提取摘要。
...
一个在线的 GIF 编辑，也可以对 MP4 视频进行解析导入。
...
苹果设备的视频播放器，可以管理和播放本地、网盘、NAS、Cloud Drive 来源的视频。
...
该生成 Swagger 或 OpenAPI 3 格式的 API 接口文档，也可当作简洁的 Postman 使用。
...
一个有意思的网站，输入一个关键词，自动生成该词的时间线，上图是输入“github”的生成结果，它的代码仓库在 [GitHub](https://github.com/zhugezifang/ai_timeline)。
...
一个在线，生成漂亮的日文排版，支持横竖版式、自动假名标注、深浅两种主题、字体配置、高亮、下划线、字体等。

[Web-Check](https://github.com/Lissy93/web-check)

![](https://cdn.beekka.com/blogimg/asset/202403/bg2024030401.webp)

开源的网站分析, 可以分析网站的 IP 信息、SSL 链、DNS 记录、Cookie、域名信息、服务器位置、网站性能等，查看 [Demo](https://web-check.xyz/)。
...
一个 JS 构建器的分析，能够查看构建产物的模块关系，并分析构建器对代码的更改，支持 Rspack 和 Webpack。

1[Photo Relay](https://github.com/zobor/photo-relay)

![](https://cdn.beekka.com/blogimg/asset/202403/bg2024030503.webp)

一个封面图片制作的在线。 [Demo](https://www.duelpeak.com/pages/poster)。
...
一个开源的软件包缓存镜像站，用于在内网加速软件包下载和镜像拉取，目前支持 DockerHub、PyPI、PyTorch、NPM 等镜像缓存服务，需要自己架设。
...
路由器操作系统 OpenWRT 的一个模块，用来为自己架设的 WiFi 服务提供认证。
...
微软的官方，用来查看 Windows 系统所有 TCP 和 UDP 连接的详细信息。

[EasyDevo](https://easydevo.boringboring.design/)

![](https://cdn.beekka.com/blogimg/asset/202402/bg2024021008.webp)

Mac 桌面应用，用来清理系统的垃圾文件，以及监控系统指标（网络、CPU、RAM 和电池等），参见[介绍文章](https://medium.com/@hi_caicai/easydevo-the-developer-tool-you-might-need-a70dfd6ee3e1)。

[FontMagic](https://github.com/leibnizli/fontmagic)

![](https://cdn.beekka.com/blogimg/asset/202402/bg2024021905.webp)

字体软件，可以将字体文件的某个字形导出为 SVG 格式，还可以转化字体格式，支持 macOS 和 Windows。

[uv](https://astral.sh/blog/uv)

Rust 语言写的 Python 包管理器，速度非常快，可以替代 pip 和pip-tools。
...
使用 rust 重写的 cp 命令，针对多核、大内存、固态磁盘、nfs 挂载目录等情况进行了优化。
...
局域网内的文件互传，只需双方打开网页，基于 WebRTC 技术。
...
本地离线翻译的 API ，不联网就可以翻译多种语言，基于 LibreTranslate 的封装，支持 Mac/Linux/Win。
...
作者修改了 ChatGPT Next Web 的源码，使其可以用于 Gemini Pro，这里是 [demo](https://chat.googlegemini.co/)。
...
只使用一张图片，就可以提取人脸，用于个性化图像合成，并支持各种不同的风格，试用 [Demo](https://instantid.org/#playground)。
...
这个可以将 GitHub Issues 转成一个静态的博客网站，单个博客使用免费。
...
一个开源的 SaaS [程序示例](https://www.smartexcel.cc/)，演示登录和支付功能的实现。
...
免费的在线简历生成，集成 ChatGPT，可以让 AI 修改、定制简历。
...
这个可以起一个服务，将 OpenAI 的 API 调用转为 Gemini Pro API 的 API 调用，从而可以使用现有的 ChatGPT 客户端，体验 Gemini Pro。

1[Calendar Remark](https://github.com/xyxc0673/calendar-remark)

![](https://cdn.beekka.com/blogimg/asset/202401/bg2024011707.webp)

这个可以标记日历，并生成分享图，适合说明私有的日程安排（比如公司活动），参见[介绍文章](https://xym.craft.me/qxAl6skGDFeVsR)和 [Demo](https://calendar.xym.im/)。
...
Vue 官方开发者的下一个迭代，旨在增强 Vue 开发者体验。
...
一个开源的画板项目，汇聚了很多创意画笔，试用 [demo](https://songlh.top/paint-board/)。
...
一款基于 Golang 和 ReactJS 的开源问答平台软件，帮你快速建立问答社区，也可用来搭建社区论坛、支持中心、知识库等。
...
它可以将 ChatGPT 请求转换为 Github Copilot Chat 的请求。只要拥有 Github Copilot 账号，就能无限制使用 ChatGPT 的 GPT-4 模型。

[Bluestone Markdown](https://www.bluemd.me/)（青石）

![](https://cdn.beekka.com/blogimg/asset/202312/bg2023123004.webp)

一个所见即所得的 Markdown 桌面编辑器，集成了 Mermaid 图形与 Katex 公式，支持明亮和暗黑风格。
...
一款基于 fabric.js 的创意图片编辑器，支持自己部署。
...
离线运行的本地语音识别转文字，基于 fast-whisper 模型。
...
一款极简的人声和背景音乐分离，将音视频文件分离为单独的人声文件和伴奏文件，完全本地化作业，无需连接外网。

[DouyinLiveRecorder](https://github.com/ihmily/DouyinLiveRecorder)

一款免费的直播录制，支持录制抖音、Tiktok、快手、虎牙、斗鱼、B站、小红书等多平台的直播视频。

1[Drawing Prompt](https://drawing-prompt.com/en)

![](https://cdn.beekka.com/blogimg/asset/202312/bg2023122705.webp)

一个文生图的辅助，可以自动扩展提示语，将简短的提示（非英语也可以）变成详尽丰富的英语提示，免费且无需登录。还支持生成随机的提示词 tag 组合，并内嵌 Fast SDXL 模型，用于预览效果。

1[cloudflare-ai-web](https://github.com/Jazee6/cloudflare-ai-web)

通过免费的 Cloudflare Worker，搭建一个你自己的 AI 平台，支持切换多个 AI 模型，试用 [Demo](https://ai.jaze.top/)。
...
AI 编程助手，支持 VS Code、 IntelliJ IDEA、PyCharm、GoLand、WebStorm 等 IDE 编辑器，参见[介绍文章](https://github.com/CatsAndMice/blog/issues/72)。

[bproxy](https://github.com/zobor/bproxy)

![](https://cdn.beekka.com/blogimg/asset/202312/bg2023121004.webp)

一款抓包代理，可以拦截 HTTP 请求，配置文件是一个 JS 脚本。
...
音乐迁移助手，将网易云音乐、QQ 音乐的歌单，迁移至 Apple/Youtube/Spotify Music。
...
支持中文、英文、日语、韩语4种语言，可在线从麦克风录制声音。

[Elog](https://github.com/LetTTGACO/elog)

![](https://cdn.beekka.com/blogimg/asset/202311/bg2023112003.webp)

该可以将写作平台（语雀/飞书/Notion/FlowUs）的内容，发布到博客平台（Hexo/Vitepress/Confluence/WordPress）等。
...
一款配置中心对比，可以比较不同配置中心的配置数据，只支持 Windows 平台。
...
用户输入文章的主题，该网页会自动生成 SEO 友好的标题、描述、关键词和常见问题解答。
...
金蝶的企业级产品设计系统，包括设计规范、设计资源、前端组件库。
...
该只用了 CPU，没有用到 GPU，也没有用到任何商业接口，无需付费。
...
一个 Rust 语言写的 JS 脚本打包器，速度很快，支持从 Webpack 移植。
...
一套开源的 Java SpringBoot + Vue 网站开发平台，可以基于它的前端和后端开发网站。
...
腾讯内部使用的一款程序远程通信框架，类似于谷歌的 gRPC，首期开源 Go 和 C++ 实现，参见[介绍文章](https://mp.weixin.qq.com/s/ODEBU6fSTZ0ixgnQxeamTg)。
...
它使用 GitHub Actions 和 ChatGPT，将仓库里面的 Markdown 文件翻译成其他语言，适合制作网站的国际化版本。
...
官网有很多作品展示，其中有几个相当可以（上图）。它应该是目前最强的 AI 网页生成器。现在还处于 Alpha 阶段，使用资格需要排队等待开通。
...
讯飞公司的 AI 大模型，可以根据提示生成 PPT 和简历，以及文档问答。
...
一款 macOS 刘海屏，将快捷键藏在刘海里面，鼠标移入时显示。
...
SDXL 是 Stable Diffusion 最新发布的画图模型。这是网友架设的体验站，不需要登陆也不需要付费，直接体验文生图。
...
用户上传 Excel 文件，该[开源](https://github.com/eosphoros-ai/DB-GPT/)让你与文件聊天，向它发出指令。

1[use autojump in vscode](https://marketplace.visualstudio.com/items?itemName=webxmsj.autojump)

VSCode 插件，用户可以使用快捷键，跳转到指定目录，类似于 z、autojump、zoxide 等命令行。
...
一个主打“听歌”功能的 Web App，有丰富的音乐资源、简洁的 UI 和方便的交互，[代码开源](https://github.com/enzeberg/tonzhon-music)。
...
国产的数据库数据结构（schema）管理，基于 Web 界面，需要自己部署，支持各种主流数据库。
...
一款开源的 PHP 应用，可以根据需要作为论坛、社交网络、CMS 系统等使用。
...
一个 iOS 应用，基于 chatGPT，实现口语化的输入来记录食物热量，例如“我吃了三颗葡萄”。
...
基于 Web 的个人笔记软件，需要自己搭建服务，可以实现与其他站点的聚合。

[mdBook](https://github.com/rust-lang/mdBook)

制作电子书的命令行，从 Markdown 生成 PDF 和 EPUB 文件，参见[教程](https://mp.weixin.qq.com/s?__biz=MzIyNDQ2MTQwOQ==&mid=2247493433&idx=1&sn=8a6f2c9975f34b9e2e7be62479f24478&chksm=e80c3037df7bb9212d320ca3fff025a81347fbc163add2a0456ea5a522ab6a836755af84ccb6&token=1949573976&lang=zh_CN#rd)。

[PDDON](https://pddon.com/)

![](https://cdn.beekka.com/blogimg/asset/202308/bg2023081005.webp)

一款在线画图，提供低代码和 AI 智能辅助。
...
一个基于 Whistle (命令行版本) + Electron 的跨平台桌面程序，用来调试各种网络请求，查看流量细节。

[Erin](https://erin-homepage.vercel.app/)

![](https://cdn.beekka.com/blogimg/asset/202308/bg2023080304.webp)

Chrome 浏览器插件，将标签显示为类似 macOS 底部 Dock 图标栏的风格。
...
Spring 性能分析，收集启动过程数据，生成交互式分析报告。
...
VS Code 插件，使用 AI 自动生成代码提交的 Commit 摘要，你就不必自己写了。它需要 OpenAI API Key。

[React1s](https://github.com/aaamoon/react1s)

![](https://cdn.beekka.com/blogimg/asset/202307/bg2023071317.webp)

浏览器插件，本地开发 React 项目时，点击页面元素，就会跳转到编辑器的对应组件位置。
...
开源的 Chrome 浏览器插件，显示网站 ICP 备案、Whois、DNS、服务器位置及运营商信息。

[艺码](https://yima.me/)

![](https://cdn.beekka.com/blogimg/asset/202307/bg2023070403.webp)

根据文字描述，将二维码转成一幅可扫描的图像的网页。
...
一个基于 Skia 的 2D 网页图形渲染库，底层使用 Rust 语言实现，前端使用 Vue 语言。你可以把它看作 SVG 的替代方案。
...
用 Go 实现的 ssh 客户端，可以记住登陆过的服务器，内置支持上传和下载文件。
...
一款开源的数据库可视化操作，基于 Java，用来平替同类付费软件。
...
开源的浏览器 LLM 模型，只要访问作者已经部署的网页，就能使用，所有数据都在本地训练。
...
一个开发者，帮你快速生成基于 AI 的应用。

1[dnrm](https://github.com/markthree/dnrm)

deno 实现的 npm 镜像源切换，每次切换都在 100ms 内，速度超级快。
...
上传一张东亚文字的图片，这个开源可以识别这些文字用了什么字体。
...
Microsoft Word 的插件，集成了 ChatGPT，用来生成、修改文本。
...
使用 Express 搭建的 Node.js 应用，用来连接 OpenAI API 进行聊天。代码开源。这是前端代码，另有[后端代码](https://github.com/KiritoCheng/openai-server)。
...
使用 AI 自动生成 SQL 语句的，支持多种数据库。用户需要提供“表结构”，并用自然语言描述想要什么数据。

[PyGWalker](https://github.com/Kanaries/pygwalker)

![](https://cdn.beekka.com/blogimg/asset/202304/bg2023041206.webp)

一个 Python 库，将 Pandas 数据帧（DataFrame）转成 Tableau 式的数据可视化互动界面。
...
一个 Node.js 爬虫库，用法灵活，功能较多，内置了 puppeteer。
...
一个 iOS 设备的 RSS 阅读器，使用 OpenAI 自动给文章添加标签，生成文章概要。

[delta](https://github.com/dandavison/delta)

![](https://cdn.beekka.com/blogimg/asset/202304/bg2023040501.webp)

`git diff`命令的代码高亮器，可以安装不同的高亮样式。
...
一个开源的软件功能（feature）管理平台，支持灰度放量、AB 实验、实时配置变更等操作。
...
一个命令行小，用来管理和切换 npm 仓库地址，类似的还有 [prm-cli](https://github.com/wangrongding/prm-cli)。
...
开源的数据探索，可以从各种数据源导入数据，然后自定义数据的查询和展示形式（图表、仪表盘、地图等等）。

[DrissionPage](https://github.com/g1879/DrissionPage)

一个基于 Python 的网页自动化，可以编写脚本操作浏览器。
...
划词翻译。选中一个词以后，它会唤起一个本机的 WebView 窗口，在该窗口中返回翻译网站的结果。
...
简易的个人 Linux 服务器管理面板（基于Node.js），前后端都开源。
...
不过，这是收费软件，需要付费购买，且手机芯片不能低于 A13。另外，由于模型不同，这个是中文版，另有[英文版 Queryable](https://apps.apple.com/us/app/queryable/id1661598353?platform=iphone)。

[密码学](https://crypto-online.cn/playground/hash)

![](https://cdn.beekka.com/blogimg/asset/202302/bg2023020402.webp)

作者将常用的一些密码学的 Web 版，比如生成密钥、哈希、解密加密等等，做在了一起方便使用。

[商务邮件生成器](https://email-helper.vercel.app/)

![](https://cdn.beekka.com/blogimg/asset/202302/bg2023020404.webp)

使用 OpenAI 自动生成商务邮件，只要提供邮件主题即可，支持中文，效果不错。

[go-get-folder-size](https://github.com/markthree/go-get-folder-size)

一个 Node.js 库，可以递归获取一个文件夹的大小，采用 Go 语言编写，要比 Node.js 原生实现快得多。
...
一个在命令行终端运行的浏览器，基于 Chromium，但是自己调用 Skia 图形库。也就是说，它不用新建窗口，就能图形展示网页，非常厉害，参见[介绍文章](https://fathy.fr/carbonyl)。
...
一个基于 Node.js 的命令行，从 DNS 服务器获取某个网站的所有 IP 地址，逐一进行延迟测试。
...
一个背景音乐 iOS App，集成了 LoFi、脑波、古典乐、白噪音等。
...
一个开源的 Web 音乐服务，需要自己架设在服务器或本地，可供多人使用，参见[介绍](https://mebtte.com/introducing_cicada)。
...
只需要简单几步配置，就能够将 ChatGPT 集成到钉钉群聊当中。用户需要有 OpenAI API 密钥。
...
一个 wCaptcha ，使用工作量证明来区分客户端是不是机器人。客户必须进行一个数学计算，大约花费几秒钟的时间，wCaptcha。

[Go Captcha](https://github.com/wenlng/go-captcha)

![](https://cdn.beekka.com/blogimg/asset/202212/bg2022121508.webp)

一个 Go 库，用于生成用户验证测试，无需键盘手动输入，支持 PC 端及移动端，参见 [Demo](http://47.104.180.148:8081/go_captcha_demo/)。
...
手机端的图床管理和文件上传下载，用于将手机照片和文件上传到各种云存储服务，基于 Flutter。
...
这是一个视频编辑软件，通过编辑字幕文件，完成视频剪切，适合课程/讲座类视频的编辑。如果视频没有字幕，也可以自动生成。
...
内网穿透 frp 的 C 语言客户端。原本的 frp 客户端使用 Go 语言，可执行文件较大，无法安装在路由器上，用 C 语言实现就可以了。
...
一个微信小程序（微信搜索“人生报告”），将周刊224期介绍的 life-stats 改造成中文版。用户输入生日，它返回一些统计数字，比如心跳了多少次，睡了多少小时。
...
一个国产的代码生成的人工智能，跟 GitHub Copilot 类似，可以将一种语言的代码翻译成另一种，并提供 VS Code 插件。

[Lama Cleaner](https://github.com/Sanster/lama-cleaner)

一个开源的 AI ，可以从图片上擦除任何不需要的物体。

[vtui-npm](https://github.com/webfansplz/vtui-npm)

![](https://cdn.beekka.com/blogimg/asset/202211/bg2022111009.webp)

一个命令行，可以互动式搜索和安装各种 JS 的 npm 模块。
...
这个开源可以将你的网易云音乐的听歌记录生成一张 SVG 图片，插入网页。
...
微软的免费笔记软件 OneNote 的 Markdown 插件，可以用 Markdown 语法输入笔记，参见[介绍文章](https://sspai.com/post/76228)。
...
一个开源的浏览器插件，可以自定义浏览器的起始页。

[MusicFree](https://github.com/maotoumao/MusicFree)

![](https://cdn.beekka.com/blogimg/asset/202210/bg2022101012.webp)

一个开源的安卓平台免费音乐播放器，使用 React Native 编写，支持自定义插件。

[pcl.js](https://pcljs.org/zh-cn/)

![](https://cdn.beekka.com/blogimg/asset/202210/bg2022101013.webp)

这个将大型 C++ 库 Point Cloud Library (PCL) 编译为 WebAssembly，运行在浏览器中，供 JavaScript 调用。它实现了大量相关的通用算法和数据结构，涉及点云获取、滤波、特征提取、识别、可视化等功能。
...
一款 iOS App，使用手机摄像头检测坐姿是否正确，可与 Apple watch 手表联动，参考[介绍文章](https://zhuanlan.zhihu.com/p/563941565)。
...
一个轻量级 Java 应用开发框架，资源占用少，支持从 Spring Boot 迁移。
...
一个显示磁盘占用情况的命令行。虽然 Linux 已经有了 ncdu，但是这个是用 Go 写的，可以跨平台（Linux、Mac & Windows）。
...
个人用户的音乐管理，可以在网上搜索歌曲，然后一键下载或者上传到云盘。
...
YY 直播的动画效果方案，涉及 iOS、安卓和 Web，包括 After Effects 插件和其他，可以先看仓库里面的系列介绍文章。
...
开发低代码的[开源框架](https://github.com/smartxworks/sunmao-ui)，内置图形化编辑器，允许开发者封装任何 React 组件，放到编辑器里面使用，参见[介绍文章](https://juejin.cn/post/7122368174808367141)。

[Auto Clipboard](https://github.com/wangmeijian/auto_clipboard)

![](https://cdn.beekka.com/blogimg/asset/202207/bg2022072803.webp)

开源的 Chrome 浏览器插件，只要选中文本，就自动复制到系统粘贴板。

[Hakuba](https://github.com/YeungKC/Hakuba)

![](https://cdn.beekka.com/blogimg/asset/202207/bg2022071801.webp)

这个可以把 GitHub 讨论区（GitHub Discussion）转变成一个静态的博客网站。并且配置了自动构建，只要论坛发生变动，就会重新构建网站。
...
在线的 JSON 格式化，方便展示、查看、编辑 JSON 数据。类似还有 [JSON Hero](https://jsonhero.io/)。
...
一个极简的 Git 帐号管理，可以用它方便地切换和管理 Git 用户。

[DBER](https://dber.tech/)

![](https://cdn.beekka.com/blogimg/asset/202207/bg2022071417.webp)

数据库 ER 图的可视化设计，用来设计数据库结构，拖拽生成模型引用关系，可以一键导出 SQL 语句。
...
这个 macOS 可以提取短信里面的验证码，自动复制到剪贴板，参考[这篇教程](https://taohan.xyz/article/maccopier)。短信需要苹果手机设定转发到桌面电脑。
...
tkinter 是 Python 自带的标准库，用来制作简单的图形界面，无须另行安装，支持跨平台运行。这个小是 tkinter 布局助手，仅需在网页上拖拽组件，就能制作出 tkinter 界面，所见即所得，自动生成 Python 代码，这里是 [Demo](https://www.codingstudy.cn/tkinter-helper/)。
...
基于 React1React-Router vReact-Hooks、Redux、TypeScript、ViteAnt-Design 搭建的一套后台管理界面，这里有 [Demo](http://hooks.spicyboy.cn/)。
...
GitHub 有时在国内不容易打开，这里有一个服务是 GitHub 的镜像加速器。提醒一下，如果要登陆 GitHub，使用这种服务会有安全顾虑，大家自己权衡。
...
Go 语言的微服务框架，内置丰富的微服务能力，可以很方便地将 Go 脚本架设成服务。
...
Windows 的便利贴小。根据 Win10 自带的便签改进而来，增加了很多功能。
...
Windows 桌面软件，用来下载 B 站视频，支持批量下载，支持8K、HDR、杜比视界，提供箱（音视频提取、去水印等）。
...
专为 TypeScript 设计的 RPC 框架，经千万级用户验证。适用于 HTTP API、WebSocket 实时应用、NodeJS 微服务等场景。

[图片美化器](https://devtool.tech/image-share)

![](https://cdn.beekka.com/blogimg/asset/220204/bg2022042016.webp)

一个网页小，可以为你的图片添加一个背景框，并添加标题。
...
一个实验性的语音编程项目，通过口述指令生成代码，这里试用 [Demo](https://pedrooaugusto.github.io/speech-to-code/webapp/)。
...
一个浏览器插件，减少摸鱼的时间和频率。打开插件后，一旦访问指定的消磨时间的网址，它就会自动统计浏览时间，达到门槛值就会弹出提醒。

[视频 PPT 提取器](https://github.com/wudududu/extract-video-ppt)

某些教学视频都在讲解 PPT 文案，或者需要导出 PPT 供以后学习，这个可以从视频里面提取 PPT，保存为 PDF 文件。
...
Go 语言写的中间人拦截，可以拦截 HTTPS 通信，带有插件扩展功能和 Web 界面。
...
这个网页可以演示 Unicode 码点如何转成 UTF-8 编码。
...
收集各种家常菜的菜谱，这是小程序版，另有 [Web 版](https://github.com/Anduin2017/HowToCook)。
...
一个同学原创的网页通知组件，代码很简单，只有41行。

[伽马影迷](https://ggt1024.com/#/productJwksExt)

一个浏览器插件，可以在 B 站/豆瓣/IMDb/烂番茄/MetaCritic 等网站上，聚合显示电影评分和第三方跳转搜索链接。
...
一个跨平台的命令行，能够同时备份本机不同浏览器的浏览记录，放进一个 SQLite 数据库，支持导出和可视化。

[Denoflow](https://github.com/denoflow/denoflow)

一个自动化工作流，只要本地安装 Deno，然后使用 Yaml 格式写一个任务配置，就能执行，相当于一个轻量级的本地化 GitHub Actions 替代品。

[腾讯柠檬清理](https://lemon.qq.com/)

![](https://cdn.beekka.com/blogimg/asset/202202/bg2022020602.webp)

腾讯官方发布的免费 macOS 系统的磁盘清理软件，一键释放磁盘空间。
...
一个基于浏览器的笔记，特点是支持双向链接，可以视作一个简单的 Wiki 式的知识管理。在线使用，无需注册和安装，数据保留在本地。
...
一个 VS Code 插件，只要在 JavaScript 脚本里面选中一个变量，就可以自动为这个变量插入一行`console.log`打印语句。
...
一个 PPT 网页应用，还原了大部分 PowerPoint 常用功能，支持 PPT 的在线编辑、演示和导出。这里有 [demo](https://pipipi-pikachu.github.io/PPTist/)。
...
一个 macOS 的网页收藏器，支持分类、标记、收集图像、添加标签，所有数据都保存在你的 iCloud。
...
一款在线的甘特图，可以直观地体现出成员、任务、时间之间的关系。
...
微软推出的 Windows 扩展集，包含不少有用的小，比如颜色选择器、图像大小调整器、快捷键指南等等。
...
一个网页小，生成文本的树形目录结构，方便将目录结构插入 Markdown 文章。
...
向服务器发出 HTTP 请求，服务器会返回请求时间。利用这个原理，这个向多个网站服务器发出请求，从中获取准确的当前时间，用于校准系统时间。它适用于无法使用 ntp 设置系统时间的场合。
...
一个前后端都开源的在线教室，企业可以用来作为自己的内部培训，或者对外的远程教学。
...
抖音前端团队推出的设计系统，支撑字节内部近千平台产品。目前提供50多个组件、国际化14种语言、支持 TypeScript、SSR 等特性。
...
一个批量压缩图片的网页，纯浏览器端压缩，不经过服务器，速度较快。
...
腾讯公司内部的开源项目，一个强大的 HTTP 接口测试，可以机器学习零代码对接口测试。
...
一键安装 Kubernetes 集群的脚本，符合国情，包含 MetricsServer 、Kuboard 、Kubernetes Dashboard 等组件。
...
macOS 应用，可以对系统播放的声音（比如浏览器、视频播放器、音乐播放器）进行语音识别，然后实时生成字幕。
...
一个网页，用来生成照明阴影效果的 Flutter 代码。这里还有 [CSS 版](https://neumorphism.io/)。
...
这个插件可以让 JetBrains 公司的 IDE，在背景里面播放视频。适合在同一个窗口，一边编码，一边看视频。
...
一个网页的语音转文字，使用 Chrome 浏览器的 Web Speech API，支持中文，[代码](https://github.com/bensonruan/Chrome-Web-Speech-API)开源，这里还有一篇[介绍文章](https://bensonruan.com/voice-to-text-with-chrome-web-speech-api/)。
...
一个将 React 的 class 组件转成 function 组件的命令行。
...
搜狗公司开源的一个 RPC 远程调用框架，使用 C++ 编写，支持多种传输协议和数据格式。
...
一个开源的数据可视化分析，支持丰富的数据源连接，能够通过拖拉拽方式，快速制作图表和仪表盘。
...
一个示例项目，作者介绍如何从零开始实现一个小型的时序数据库（TSDB），也就是专门用于时间序列的数据库。
...
一个面向企业用户的低代码平台，帮助企业不用写代码，就能搭建管理系统、业务平台、企业官网等，有免费使用的版本。
...
一个网页，使用 WIFI 的名称和密码，生成一个二维码，手机扫一下，就能登陆这个无线网络。
...
一个企业级的项目管理和协作平台，集成了工作台、看板、思维导图、项目文档、仪表盘、文件等多种功能，还可以接入代码构建与交付，20人以下的团队免费。

[kit](https://github.com/cuishuang/kit)

![](https://cdn.beekka.com/blogimg/asset/202107/bg2021072109.jpg)

一个 PHP 小，用来列出 MySQL 数据库的表结构信息。
...
Kindle 笔记导出，可以在线导出，然后写入笔记软件 Flomo。
...
成都市2021年7月6日公布了《城市轨道交通线网规划图》，但是不方便缩放操作。作者写了一个静态页面将其和百度地图结合，方便查看未来地铁是否经过了你的家门。
...
一个将文字或代码转换成图片的网页，可定制项多，主题、水印、阴影特效等都能自定义。
...
这个可以将你在各种网站上的活动记录，生成 GitHub 方块热力图或 skyline 图。

[Apitest](https://github.com/sigoden/apitest/blob/master/README.zh-CN.md)

一个 API 测试的自动化，所有测试用例可以写在一个类似 JSON 的配置文件里面。

[hacker-feeds-cli](https://github.com/Mayandev/hacker-feeds-cli)

一个命令行，可以查看 Hacker News、 Reddit、V2EX、GitHub Trending 等资讯网站。
...
大家用桌面浏览器访问这个网站，会列出你的电脑安装了什么应用（上图）。这样做的目的，是生成一个用户指纹，从而追踪用户，作者写了一篇详细的[文章](https://fingerprintjs.com/blog/external-protocol-flooding/)，解释这是怎么实现的。

[Chanify](https://github.com/chanify/chanify/blob/main/README-zh_CN.md)

一个开源的 iOS 设备消息推送，分为服务器和 iOS App 两个部分。安装以后，可以将文本/图片/文件等各种消息推送到 iOS 设备。
...
一个在线，用来分析、修改 APK 文件（安卓应用安装包），以及提取 APK 图标。
...
使用 Markdown 编写简历的在线，支持模板与实时预览，可以导出为 PDF 文件。

1[木及简历](https://resume.mdedit.online/)

![](https://cdn.beekka.com/blogimg/asset/202105/bg2021051903.jpg)

编写简历的在线，与上一个软件类似，有多个模板可选择，源码见 [GitHub](https://github.com/hua1995116/react-resume-site)。
...
小程序框架 Taro 的一个好用的路由库。

[Golang-iDevice](https://github.com/electricbubble/gidevice)

一个 Go 语言开发的，用来与 iOS 设备通信，可以作为库使用，也可以在命令行独立使用。
...
一个 JavaScript 代码的 AST 抽象语法树的处理，便于方便地转换代码。

[Storyteller](https://storyteller.webzard.io/)

![](https://cdn.beekka.com/blogimg/asset/202104/bg2021042508.jpg)

如何写软件教程，是一个令人头痛的问题。Storyteller 是一个在线，提供一个教程编辑器，以无代码的方式制作交互式教程。
...
一个检查中文 Markdown 语法风格的命令行，比如英文字母与全角字符之间有一个空格。
...
一个实验性桌面程序，使用百度飞轮和 Electron 实现的 AR 剪贴和复制。手机先对准某人，再对准电脑屏幕，即可把他/她复制粘贴到桌面程序里面！

[rss_everyday](https://github.com/GuangzheJiang/rss_everyday)

一个 GitHub Actions 模板，每天定时运行，将 RSS 内容推送到 Telegram 频道。
...
一个使用 EXIF 信息的图片文件批量改名，支持 macOS 和 Windows。
...
一个 GitHub Actions 脚本，定时运行脚本机器人，将非英语的 Issue 自动翻译成英语。
...
这个小可以把文章转成视频，文字显示在黑底的图片，同时自动生成语音，详细可以[作者的介绍文章](https://zhuanlan.zhihu.com/p/353515449)。

1[Bing Wallpaper](https://github.com/niumoo/bing-wallpaper)

![](https://cdn.beekka.com/blogimg/asset/202103/bg2021030307.jpg)

这个项目使用 GitHub Actions，收集 Bing 搜索引擎每天的壁纸，提供下载，详见[这篇说明文章](https://www.wdbyte.com/2021/03/bing-wallpaper-github-action/)。
...
一个使用 Python 写的 Linux Shell，可以在 Bash 代码里面使用 Python 语法。
...
Python 的数据可视化图表生成，基于蚂蚁集团开源的 G2Plot 封装。
...
一个在线，可以将 Swagger 文档（yaml 或者 json 格式）导出为PDF、Markdown、EPUB、HTML 等格式。
...
蚂蚁集团数据可视化 AntV 旗下的图编辑引擎，用来在网页上生成 SVG 图形。
...
很多在线视频都是许多 .ts 文件组成的，这个在线可以输入 .ts 文件的网址，然后将它们合并。
...
这个在线可以将 C 语言难懂的类型声明（比如`int (*(*foo)(void ))[3]`），转成浅显的英语。
...
Cloudflare 推出的静态网站构建和托管服务，目前正在 beta 测试，可以申请参加。
...
一个小而全的 Java 类库，通过静态方法封装，并提供详细完整的中文文档和注释，使Java拥有函数式语言般的优雅。
...
Nvidia 显示卡配合 FFmpeg 进行视频编码时，有并发数量限制（3路并发），这个补丁可以突破这一限制。
...
一个开源的在线，可以将文章发布到 IPFS 网络。一经发布就无法再删改，理论上可以永久储存。
...
一个语音的集成软件，文档教你如何在树莓派上使用 Node.js，搭建自己的语音助手，可以识别语音，也可以将文本转为语音。
...
国产的在线表格数据库，类似国外的 Airtable，提供了强大电子表格功能，比如多表关联、一对多的自动聚合运算、看板视图、多人实时协作、行级数据权限等等。
...
一个 macOS App，可以直接把 Java 源码转换为markdown/asciidoc 格式的 API 接口文档，免去繁琐的手写过程。
...
通过 GitHub 的 Action 定时执行脚本，获取 GitHub Trending Top 10，并且记录到了项目的 issue 里。用户可以通过 watch GitHub 仓库或者 RSS 订阅更新。
...
Facebook 推出的一个静态代码分析，可以分析 Java、C++、Objective-C、C 代码里面的错误。
...
数据大屏生成，后端使用 flask。只要传一下数据，就可以制作出数据大屏，适合不了解前端的数据工作者。
...
一个安卓相机 App 的源码，使用 OpenGL ES 3.0 实现 30 种相机滤镜。
...
前苹果邮件客户端工程师 Neil Jhaveri 推出的 Gmail macOS 原生客户端，外观简洁大方，十分契合原生风格，参考[介绍文章](https://www.cnbeta.com/articles/tech/1027397.htm)。
...
ELK 平台（Elasticsearch，Logstash，Kibana）的定时任务管理，可以聚合处理日志，生成报表信息，通过邮件/钉钉等方式发送通知。
...
一个完整的大数据实践项目，通过 hive 分析 nginx 日志，将分析结果通过 flask + echarts 进行可视化展示。

[go-zero](https://github.com/tal-tech/go-zero)

一个国产的 web 和 rpc 微服务框架，含极简的 API 定义和生成 goctl，可以根据定义的 api 文件一键生成各语言的调用代码。
...
为开源项目提供免费的`js.cool`的子域名，类似`js.org`，支持 CNAME 绑定。
...
一个高性能的 GraphQL 服务器，提供 Postgres上开箱即用的实时GraphQL API。
...
一个可以自己部署的服务，生成个人 GitHub 账户的统计，可以插在自述文件里面。这里还有另一个[类似的服务](https://github.com/athul/waka-readme)。
...
国内连接 npm 源服务器很慢，tbify 这个可以自动将 npm、yarn 等包管理连上国内的淘宝镜像，对和系统都是非侵入性的。

[easy-monitor](https://github.com/hyj1991/easy-monitor)

![](https://www.wangbase.com/blogimg/asset/202007/bg2020072311.jpg)

Node.js 应用的性能监控，可以定位线上故障。代码开源，支持私有部署。
...
一个将个人的 LeetCode 答题记录汇总的，展示题号、题目、你的解答、 题目通过率等，然后一键生成一个 Hugo 网站。

1[钉钉的非官方 Linux 版](https://github.com/nashaofu/dingtalk)

钉钉官方没有提供 Linux 版，作者用 Electron 和钉钉网页版打包了一个。
...
Kindle 等电子书阅读器的 RSS 推送服务，绑定 Inoreader 帐号。
...
一个纯前端的在线图片格式转换，可以快速将图片转为另一种格式，并且提供多个参数可以调节。

[tauri](https://github.com/tauri-apps/tauri)

一个类似 Electron 的跨平台桌面应用开发框架，但是基于各平台自带的 WebView，所以应用打包出来的体积比较小。
...
使用 CSS 生成三角形是网页设计常见的需求，比起使用图片，CSS 生成的三角形具有更好的可控性和灵活性。本支持自定义三角形大小、颜色、方向、边框等属性。
...
一个远程调试网页的，只要在网页里面加载这个库，就可以远程打开 Chrome 开发者，主要用于调试手机端网页。
...
微软官方的命令行 Windows 包管理，估计是为了从 Linux 和 macOS 系统抢夺开发者。不知道能否最终取代 Chocolatey。

[qqwry2mmdb](https://github.com/leolovenet/qqwry2mmdb)

为抓包 Wireshark 引入纯真网络 IP 数据库的转换，用来查看 IP 归属地。
...
一个点对点的文件在线传输，利用 Webrtc 在两台电脑之间建立直接连接，代码[开源](https://github.com/saljam/webwormhole)。
...
一个社会化的 RSS 阅读器，除了常规的 RSS 聚合功能外, 还加入了一些社会化的属性。
...
一个轻量的电子邮件推送开源，可以与网站的用户反馈、留言等功能进行集成，将数据填入模板，投递到指定的邮箱。
...
专业的 Audition 软件主要面对很多专业人士，里面的操作比较复杂，对于普通的人来说要想完成一个简单的功能操作比较繁锁，所以这个网站兼顾了准专业性和傻瓜式的操作。
...
中文的字体文件都非常大，网页全部加载不现实。这个可以从字体里面，选取网页需要的字符，生成新的字体文件。

[xterm-player](https://github.com/JavaCS3/xterm-player)

一个基于 Web 的命令行终端录制回放，解决了分享命令操作时，无法拷贝文字以及视频体积较大的问题。
...
React 的组件可视化拖拽页面编辑与代码生成，让不会 React 技术栈的人员可以通过拖拽生成页面。
...
一个实现番茄工作法（将工作划分为25分钟的片段）的 Python 小脚本。

[Fluttify](https://fluttify.com/)

一个 Flutter 框架的插件开发，解决了插件开发过程中的类型安全，编码繁琐等痛点，目前已有的典型案例有[高德地图插件](https://github.com/fluttify-project/amap_map_fluttify)。
...
一个超级轻量的静态博客系统，使用 Kotlin 语言。

[akshare](https://github.com/jindaxiang/akshare)

一个基于 Python 的开源金融数据接口库，目的是实现股票、期货等金融产品从数据采集、数据清洗到数据下载的，满足金融数据科学家、数据科学爱好者在数据获取方面的需求。
...
Chrome 浏览器插件，可以在 Spotify Web Player 里面显示歌词，歌词来自网易音乐的接口。
...
一个通用的 JSON 解析算法，简单高效快速，提供 C# 和 C 的实现，也可以轻松用其它编程语言实现。
...
一个 Hexo 的主题，可以用来制作速查表（cheatsheet）网站，参见 [devhints.io](https://devhints.io/) 的例子。
...
Chrome 浏览器插件，可以三维显示 GitHub 的提交统计。
...
一个安卓的 SSH 服务器应用，使你可以 SSH 登录到安卓手机。
...
一个安卓 App，技术阅读的入口，汇聚了国内外技术博客、产品动态、公众号文章。
...
微软公司发布了使用 Chromium 核心的新 Edge 浏览器，可以从本地的谷歌浏览器直接导入书签，历史等信息。还能从微软自己的或谷歌的扩展商店下载插件。
...
Linux 系统的虚拟桌面 App，可以起一个 VNC 服务，然后用 iPad 或其他设备的 VNC 客户端连接上去，就可以形成扩展桌面。
...
舒尔特方格是数字1-25随机分布在25个方格里面，用户按照从小到大的顺序，依次指出每个数字的位置，根据耗时判断注意力水平的高低，详见[百度百科](https://baike.baidu.com/item/%E8%88%92%E5%B0%94%E7%89%B9%E6%96%B9%E6%A0%BC/5372437)。这个游戏最好在手机上玩。
...
一个开源的容器管理平台，构建在 Kubernetes 之上，帮助企业管理容器环境。
...
国人开发的适合日本自由行的交通换乘 App，特点是全中文搜索显示，可一键切换查看日语站名，支持日本所有的地铁、电车、新干线，无广告。
...
这个在线可以把隐藏文本写入图片文件。图片依然可以正常使用，并且外观不发生变化，用户不会发现里面隐藏了加密信息。
...
一个 Go 语言脚本，每天向指定邮箱发送一封邮件，里面是天气、名言、诗歌。

[leonsans](https://github.com/cmiscm/leonsans)

一款用 JavaScript 脚本绘制的英文字体，只要网页插入脚本，就能使用这个字体，即字体使用脚本画出来的，特别适合制作字体动画。
...
该项目把游戏做成操作系统，打包成镜像，装到U盘，插入电脑开机启动，就能直接玩游戏。
...
扫描一个目录，给出目录结构、文件大小等统计信息的桌面。

[文本插入零宽度字符的](https://we.laogongshuo.com/)

这个在线小，在每个字符之间插入一个零宽度字符，用来逃过文本过滤系统，正常阅读不受影响。
...
一个开源的在线，从上传的图片里面提取两种主色，生成一张渐变的背景图，可以下载。
...
一个开源，可以查看 GitHub 仓库的 Star 变化情况，生成折线图。
...
小程序跨平台开发。一键实现微信小程序项目到支付宝小程序的迁徙，不再为重复开发而烦恼。
...
该网站收集了程序员需要的10多个在线小，包含JSON 格式化、Base64 编解码、颜色值转换、图片压缩、正则测试、时间戳转换、HASH 计算等。
...
用户输入一个字符串，在线显示各种哈希算法的运算结果。

1[logForShell](https://github.com/7ippo/logForShell)

Bash 的日志，出错时打印调用堆栈，方便快速定位问题和源文件。
...
一个国产软件，macOS 系统的多视图文件管理器，支持很多特色功能。让你摆脱多窗口来回切换的繁琐，和拖拽时找不准目标的尴尬。
...
Writeathon 是一款在线写作，基于极简理念，提供简洁、高效、专注的写作体验。
...
由于 iOS 平台的限制，iSH不能从应用市场直接安装，需要使用TestFlight安装beta版本。
...
Mac 系统的桌面图片上传，用于将本地图片上传到图床，功能强大。
...
一个查询字母缩写含义的命令行，比如缩写 OS 代表 Operation System，数据来自[abbreviations.com](https://www.abbreviations.com)。
...
国产的静态编程语言，追求简单可读，目前支持转码为 C#/Go/TypeScript。
...
GitHub issues 备份，只要填入仓库地址就可以下载压缩文件包，一次性下载所有 issue。
...
矩阵数据结构的 JavaScript 库，使用了底层的二进制接口，性能较好。
...
基于深度学习的代码补全，号称支持所有语言。

1[licia](https://github.com/liriliri/licia)

这个库收集 Web 开发的各种 JavaScript 小，使得开发者只用加载它一个库即可，跟 lodash 的思路有点像。
...
创造者出售数字作品的平台。你上传作品，设定价格，在这个平台出售。
...
一个开源服务，可以统计 GitHub 仓库的 Readme 文件的阅读次数，然后生成 badge 徽章，以便挂在仓库里。
...
一个浏览器电子表格的 JS 库，与 Excel 兼容。

[Zvm](https://github.com/5A59/Zvm)

一款 Python 实现的 "超轻量级" JVM，代码简单，结构清晰，方便学习 JVM 原理。
...
将 Markdown 文档转成幻灯片，在浏览器演示的。

1[LMGTFY.com](https://lmgtfy.com/)

该网站用动画演示搜索引擎的使用方法。新人经常提出一些简单的问题，你可以把该网站发给新人，帮助他学习如何使用谷歌。LMGTFY 是 let me google it for you 的缩写。
...
一个网页，可以将 ASCII 字符的公式转为数学字体显示。

[Table Convert Online](https://tableconvert.com/)

一个网页，可以在线表格转成 Markdown、JSON、HTML、XML、YAML 等数据格式，并支持格式之间互相转化。
...
五笔输入法的一个练习，可以根据训练者的情况，动态生成练习内容。

[APIJSON](https://github.com/APIJSON/APIJSON)

自动化接口管理，后端自动生成接口文档，前端可以定制任何数据、任何结构。
...
在线的思维导图，使用 React Hooks 与 flex 布局实现。

1[正整数因式分解在线](https://helloacm.com/tools/integer-factor/)

一个网页小，可以将正整数分解成一系列质数的乘积。该服务还提供 API。
...
一个开源的命令行文件上传服务，不超过 512MB 的文件可以免费保存30天。
...
开源的 Chrome 浏览器插件，用来批量管理浏览器打开的网页标签。这里还有一个类似的插件 [SmallTab](https://chrome.google.com/webstore/detail/smalltab/npdfjbmjfdleclpgodndpepkecjfmnkp)。
...
腾讯推出的，几行代码就可以为页面加入一个互动社区，可以接入应用程序、小程序、WEB、H微信公众号等各端。
...
这个开源可以为任何 GitHub 目录或文件，生成下载链接，即点击下载代码库的任意子目录。
...
一款自研的软件研发管理 SaaS 系统，主要面向国内的中小互联网软件企业及开发团队。
...
小众语言的社区，可以建立各种主题的子社区。[CodePlantes](https://github.com/coderplanets/coderplanets_server) 是开源的，服务端使用 Elixir 语言编写。
...
Basic 语言开发，支持几乎所有平台，源码会被转译为对应平台的代码，比如Java、Objective-C、JavaScript。
...
Bash 命令的可视化解释。遇到复杂的 Bash 命令，可以输入到这个网站，查看该命令的解释。

[Project Showcase](https://projectshowcase.me)

该根据你的 GitHub 个人项目，自动生成一张个人介绍页面。
...
端到端的三维人脸重建，基于 MobileNet 网络框架，在 CPU 上能达到实时。

[super-inspire-end](https://github.com/super-inspire/super-inspire-end/blob/master/docs/README-zh.md)

使用 Docker 起一个 Linux 系统的容器，然后就可以在浏览器访问 Docker 里面的 Linux 系统。
...
在线编写 ipynb 文档的，可以在网页上直接编写和执行 Python、R、Julia 代码。
...
PDF 文件即时翻译。只要选中 PDF 文件里面的文字，就自动调用谷歌翻译的 API，将其翻译成中文，粘贴出来就是中文。
...
一个将 curl 命令转为 Python、Node.js、R、PHP、Go 代码的在线。

[es_data_export](https://github.com/760515805/es_data_export)

ElasticSearch 数据导出，目前支持json、txt。20亿的索引表中导出4000W数据只用到不到15分钟。

[Musicoco](https://github.com/DuanJiaNing/Musicoco)

一个 Android 音乐播放器，适合初学者借鉴学习。
...
一个上世纪80年代的电视的模拟器，可以切换频道和进行其他操作。

[asciinema](https://asciinema.org/)

一个录制、分享命令行操作的，支持Linux, macOS

[C4](https://chai2010.cn/awesome-wasm-zh/examples/c4/)

一个在浏览器运行的 C 语言运行时，功能较为简单。
...
输入一个产品的名字，该会根据谷歌的自动搜索提示，生成一张图，上面是经常与该产品进行对比的其他产品。
...
统计代码行数的，可以用于压缩文件（如zip tar等），包括空行和特殊字符的处理与统计，生成多种格式的结果。安装简单且轻量，对于阅读源码，分析项目代码的场景非常实用。

[ssh-chat](https://github.com/shazow/ssh-chat)

Go 语言实现的 SSH 服务端，用于登录用户之间的聊天，可以用 `ssh chat.shazow.net` 命令体验。
...
俄罗斯搜索引擎 Yandex 的图片 OCR 服务，可以将图片里面的文字翻译成另一种文字。
...
Chrome 浏览器的扩展，使用类似 Vim 的快捷键，通过键盘操作浏览器。

[Squoosh](https://squoosh.app/)

谷歌推出的图像压缩，代码开源。官网可以在线压缩图片，而且可以转换图片格式（比如 webp 转 jpg）。
...
Linux 系统 cd 命令的替代，可以记住常用路径，然后只要输入路径的一部分即可。
...
美国一家创业公司发布家用 DNS 服务器，可以提供局域网的 DNS 服务，最大功能是自动屏蔽广告。它的软件是[开源](https://github.com/AdguardTeam/AdGuardHome#installation)的，也可以安装在其他硬件上（比如树莓派）。

1[百度脑图](http://naotu.baidu.com/)

一个免费、开源、好用的思维导图。[离线版](https://github.com/NaoTu/DesktopNaotu/)可以跨平台使用，支持 Windows、Linux、Mac OS。
...
Chrome 以吃内存著称。这个插件可以冻结不使用的 tab，以释放内存。

[flyer-maker](https://github.com/vancefantasy/flyer-maker)

flyermaker 是一个 Java 项目的脚手架，支持 SpringBoot 和 SpringFramework，它允许动态生成工程代码和单元测试。
