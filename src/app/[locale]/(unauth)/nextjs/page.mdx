...
1. 服务端组件中使用的子组件可以使用 `'use client'` 吗？
   ...
   ```tsx {{src/components/SessionProvider.tsx}}
   'use client'
...
**app/api/[...nextauth]/route.ts 等效于 app/api/[...nextauth].ts**

### 如何写 api 中接口

导出什么函数就接收什么类型 method 的请求
...
使用 [next-auth](https://next-auth.js.org/) 获取当前用户信息
...
### 如何在 pages router 中使用 next-auth

### 如何在 app router 中使用 next-auth
