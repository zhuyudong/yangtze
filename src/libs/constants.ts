...
import type { VariantIdsByType } from '@/types/subscribe'
...
export const ROLES_LIMIT: { [key in Role]: number } = {
  0:
    (process.env.NEXT_PUBLIC_COMMON_USER_DAILY_LIMIT_STR &&
      Number(process.env.NEXT_PUBLIC_COMMON_USER_DAILY_LIMIT_STR)) ||
    10,
  2:
    (process.env.NEXT_PUBLIC_MEMBERSHIP_DAILY_LIMIT_STR &&
...
export const BOOST_PACK_EXPIRE =
  ONE_DAY * Number(process.env.NEXT_PUBLIC_BOOST_PACK_EXPIRE_DAYS || 7) // 7天，购买加油包的使用期限 7 days, usage duration for a purchased boost pack
export const BOOST_PACK_CREDITS = Number(
  process.env.NEXT_PUBLIC_BOOST_PACK_CREDITS || 100
) // 每次购买加油包获得的次数  Number of uses received per boost pack purchase
...
export const getSinglePayOrderKey = ({
  identifier
}: {
  identifier: string
}) => {
